...
| [T-1](#t-1) | 23 個檢驗代碼待臨床確認 | 檢驗醫學部／職業醫學科 | **已回覆（2026-07-27）；分批處理中** |
...
**現況（2026-07-29 更新：已回覆，分批處理中）**　檢驗醫學部（張永達醫師，
2026-07-27）與職業醫學科已回覆確認單。依回覆分批處理：
...
- **批 2（已處理）**：需**換碼／加碼**者中之第 2-2/2-3/2-4 題、第 3 題、第 7 題——尿氟
  `34304-6`（改莫耳）＋加質量碼 `5605-7`；AFP `19177-5`（改莫耳）＋加院用質量碼 `53962-7`；
  同半胱胺酸 `2428-1`（改質量）＋加建議優先莫耳碼 `13965-9`；Anti-HBs `22322-2`/`5193-8`
  標示對調回正確位置＋加建議定量碼 `65633-0`；巨核細胞 `70028-6`（官方為細胞核）換為
  臨床建議之 `62858-6`。**5 個新候選碼（`5605-7`/`53962-7`/`13965-9`/`65633-0`/`62858-6`）
  一律標「待 CI `$lookup` 覆核六軸」，未照單全收臨床建議之碼**——此即本 IG 之稽核紀律；
  下一輪 CI 之 Wrong Display Name 訊息即該五碼之官方定義，據以確認或修正/剔除。
  另一併更正批1 兩筆顯示名轉抄誤（`43371-4` 多一個 sp、`33914-3` 誤用舊式長名），依
  `display-verification-report.csv` 之官方字串。
- **批 3（待處理，自尋碼）**：第 1 題 9 個尿沉渣碼（院方報「每 µL」＝每體積，官方均為
  每面積，須整批換體積碼）、第 2-1 題尿錳 `42221-2`（院方報質量 µg/L，惟該碼官方為莫耳，
  須加/換質量碼）、第 4-2 題 EBV VCA IgA `9633-9`（院方以 EIA 報 `U/mL` 定量，官方為 IFA
  效價，須換定量碼）——均須自尋碼並經 `$lookup` 覆核。
- **仍卡住（1 碼，需裁示）**：第 4-1 題 H. pylori IgG `5176-3`——回覆為「本院健檢未執行」，
  該院無報告型態可據以判定定性/定量。**三選一待裁示**：(a) 自本 IG 移除該碼；(b) 保留並逕採
  LOINC 官方定量顯示名（`[Units/volume] ... by Immunoassay`）但無臨床背書；(c) 另問一家有執行
  該項之院所。決策前該碼之顯示名不符刻意保留。
