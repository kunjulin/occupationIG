...
- **批 1（已處理，5 碼）**：代碼經確認**正確、僅顯示名漂移**者——沙門/志賀氏菌培養
  ...
  `98979-8`）亦一併統一。
- **批 2 起（處理中）**：需**換碼／加碼**者（第 1 題 9 碼、第 2 題 4 碼、第 3 題、
  第 4-2 題、第 7 題）——每個候選碼須經 CI 術語伺服器逐碼驗證六軸後方採用，**不照單全收**
  臨床建議之碼（此即本 IG 之稽核紀律）。
- **仍卡住（1 碼）**：第 4-1 題 H. pylori IgG `5176-3`——回覆為「本院健檢未執行」，
  無報告型態可確認，須另問有執行之院所或裁示（移除／保留官方定量顯示名）。
