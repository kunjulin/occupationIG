...
| [T-1](#t-1) | 23 個檢驗代碼待臨床確認 | 檢驗醫學部／職業醫學科 | **批1–3 已處理；餘 2 項 LOINC 缺口＋Q4-1 已裁示** |
...
- **批3 遺留之兩項 LOINC 缺口（待裁示）**：
    - **精子（尿）定量**：LOINC 無「`[#/volume] in Urine by Automated count`」之精子碼（僅 `[Presence]`/鏡檢），
      `53324-0` 保留為官方每面積碼；全尿自動定量為 LOINC 缺口。
    - **EBV VCA IgA 定量（EIA）**：LOINC 無「capsid IgA `[Units/volume]` by Immunoassay」碼
      （僅 IgG `5157-3`／IgM `5159-9` 有），院方之 EIA／U/mL 定量無對應碼；`9633-9`（IFA 效價）為現有
      最接近之同分析物碼。兩缺口之處置（請院方改採有碼之方法／向 Regenstrief 申請新碼／維持現狀）待裁示。
