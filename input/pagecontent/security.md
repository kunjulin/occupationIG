...
凡涉及主管機關或平台端架構決定者，一律指向[未決事項](https://github.com/kunjulin/occupationIG/blob/main/docs/known-limitations.md)，不於本 IG 片面認定。
...
**存取控制政策不在本指引範圍。** 何人於何種身分、何種情形下得以存取健康檢查紀錄，
屬**法規、主管機關函釋與各機構內部政策**之範疇，非資料交換規格所能或所應決定。

此與 FHIR 標準本身及其母規範之立場一致。臺灣核心實作指引（TW Core IG）之
〈安全性〉頁載明：

> 系統可以（MAY）透過加密和相關存取控制來保護資料的機密性。
> **所使用的策略和方法不在此規範的範圍內。**
>
...
以雇主端為例：本指引**不規定**雇主有無權限存取健康檢查資料（此屬 (a)）；
但本指引**明確規定**雇主端封包
（[TWHA-Composition-EmployerSummary](StructureDefinition-TWHA-Composition-EmployerSummary.html)）
之 `section` 採 **closed slicing**，**結構上不含檢驗 section**（此屬 (b)）。
後者是驗證器會擋下來的結構約束，不是原則宣示。

> 兩者不可混為一談：「本指引不規定誰能看」與「本指引規定這份文件裡不放檢驗值」
> 同時成立，且互不矛盾。

**保存期限**（《勞工健康保護規則》第 19 條）採同一原則處理——屬各機構依法自行辦理之
行政義務，與資料交換結構無涉，已界定為**不列入本指引範疇**
（[M-6](index.html#before-you-start)）。
...
- `Provenance.signature.who`：簽署醫師，繫至其醫事人員證書字號——**該識別碼之命名空間
  尚未定案（[T-2](https://github.com/kunjulin/occupationIG/blob/main/docs/known-limitations.md#t-2)）**，故簽署者之機器可比對身分待 T-2 解決後方能完備。
...
（見 [M-10](https://github.com/kunjulin/occupationIG/blob/main/docs/known-limitations.md#m-10)）。本節之責任是把取捨寫明，不是迴避。
...
以上三者之落地屬「**稽核與同意機制之選型**」範疇，統一登記於
[未決事項 M-10](https://github.com/kunjulin/occupationIG/blob/main/docs/known-limitations.md#m-10)。**須留意該項所指之待決僅為機制選型——
雇主端可取得之資料範圍已於 §2.2 定案且可驗證，不在該項待決範圍內。**
實作端在機制選型定案前，應依《個資法》與衛福部規範自行處理，並保留可日後對接之欄位。

**保存期限**（第 19 條）之結構化亦未於本期實作——其起算點之法定解釋尚未確定
（[M-6](index.html#before-you-start)），在解釋確定前結構化等於把未定之法律解釋寫死；
詳見[背景與法規](background.html) §3.1.1 與 [M-7](https://github.com/kunjulin/occupationIG/blob/main/docs/known-limitations.md#m-7)。
