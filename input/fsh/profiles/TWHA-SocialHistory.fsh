...
// 原案欄名為「嚼檳月數」，經**委託單位（工業技術研究院）2026-08-21 指示**確認
// **其語意為「嚼了多久」（嚼食持續期間）**，而非與吸菸列對稱之「戒檳月數」
// ——故對映至本欄，不是 component[cessationDuration]。
//
// ⚠️ **依據之來源與權責（v0.10.0 更正）**：本項之依據為**委託單位指示**，
//    **不是國民健康署答覆**。該單位並表明「本項非國健署需確認事項」，
//    理由是欄位語意之字面解釋屬委託單位可逕行認定之範圍。
//    v0.6.0–v0.9.3 期間本處記為「經主管機關答覆確認」，**該敘述不實**：
//    國民健康署從未就此答覆。「經主管機關確認」是規範文件裡效力最強的措辭之一，
//    誤用會使該署日後讀到與事實不符，故逐處更正。
//    ⚠️ 仍待國健署者為**21 列清單本身之法定效力與正式公告生效時點**（M-5），
//    與本項之語意解釋是兩件事，不可混為一談。
//
