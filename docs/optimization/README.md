...
| [JOB-30](JOB-30-publish-readiness-and-governance.md) | 先行公布供臨床試用之整備：〈未決事項〉頁分層（A 部）＋ 治理分軌 Level 1／Level 2（B 部） | **P0** | 治理／遵從性／對外呈現 | M（2–3 人日） | 無（v0.4.0 已解除原 JOB-29 相依） | ✅ **已執行（v0.5.0）**：A／B 兩部一批完成。§7 合規層級（Level 1 Core 上傳合規／Level 2 勞工健檢涵蓋，後者以前者為前提而反之不然）＋ §7.0 範疇聲明（本指引之主管機關只有國健署一個，未受職安署委任）；101 個 artifact 加權責標籤（國健署 24／規則附表 50／技術規格 27）與 `standards-status`（Level 1 之 24 件 trial-use，餘 draft），新增登記表 `governance-map.js` 與閘門 `check-governance-tags.js`（7 組自我測試含 2 組正向對照，已掛入 `npm run verify`）；open-issues 一次加兩欄 26 項全填、已結案 6 項移頁尾、標題改「已知限制與試用須知」（**檔名與錨點未動**）、明細移 `open-issues-detail.md`。**三處實測與評估稿不符並已具名更正**：Core 逐列展開實得 **20 列**（非 21，主項 16 相符，已列入 M-5 待國健署確認）、具名錨點實測 **23 處**（非 24，站內連結 31 處相符）、頁面 803 → **767 行**（未達 400，理由見 §7.3：壓到 400 只能刪「若決策不同」或刪對試用者之揭露，兩者分別為 CLAUDE.md §6 與本 JOB §2.1 所禁）。T-11 之權責歸屬依既有「決定者」欄改列（與初步歸類矛盾）。嚼檳系列 `experimental` 維持 true。**待 CI 驗證**：`standards-status` 於 ValueSet／CodeSystem 之 context、以及 `qa-baseline.json` 之實測增量。 評估稿見本檔 §1–§6（v0.4.1）**更正**：「Core 21 列唯一缺口為嚼檳狀態」已因 v0.4.0 不成立，21 列現已全數具備承載，故 A／B 兩部得一批完成。A 部不整頁移出網站（實測 31 處連結、24 處具名錨點、40 處原始碼引用），改標題不改檔名＋加「對試用的影響」欄（26 項中僅 M-5／M-8／M-11 影響欄位內容）＋已結案 6 項移頁尾＋續辦移 `docs/`。B 部新增 §7 合規層級（Level 1 依國健署欄位規範／Level 2 依勞工健康保護規則附表）、逐 artifact 權責標籤與 `standards-status`、一覽表加權責歸屬欄。**已更正一處越權瑕疵**：不得把職安署列為本指引之治理機關（該署未參與、未委任），改標為「內容依據」並加範疇聲明；本指引之主管機關只有國健署一個。另因國健署「嚼檳定案即同意 21 項」而增 §3.4 M-5 結案路徑（五項送簽確認事項；**取得書面前不得變更 M-5／`experimental`／Level 1 成熟度**）。**待裁示後實作** |
| [JOB-31](JOB-31-qa-baseline-and-registry-gaps.md) | v0.6.1 線上複驗：基準線鬆動 61 筆、25 筆未具名 WARNING、ConceptMap 未納登記 | **P1** | QA 治理／閘門 | S（0.5–1 人日） | 無 | 📋 **評估（v0.6.2）**：自 gh-pages 取已發佈產出，以 `qa.txt`（非 `qa.html`——後者重複渲染會高估）複驗。**通過**：22 個具名類別零回歸；101 件權責登記全量比對相符（24／50／27，不符 0）；§7 範疇聲明與 open-issues 26 項均已上線。**發現**：(1) `totals.warn` 上限 152 vs 實測 91，鬆 61 筆未依慣例下調；(2) 91 筆 WARNING 僅 66 筆具名，25 筆未具名——含 10 筆 Composition 走訪不可達、7 筆 ImagingStudy 值集、4 筆缺 subject/effective[x]（`should have a performer` 措辭不同抓不到）、2 筆 UCUM `{quid}`（刻意）、2 筆 ShareableConceptMap；(3) 已發佈 103 件 vs 登記 101 件，`TWHealthCheckLaboratoryMap`（功能上屬 Level 1）等三件因 `scanFsh` 排除 `Instance:` 而未登記。另建議**共用技術結構 27 件不標 `draft`**（會使 Level 1 產生大量 `Reference to draft` 並觸發閘門）。**待裁示後實作** |
| [JOB-32](JOB-32-betelnut-observation-code-semantics.md) | 委員意見：`698188003` 為 finding，不宜作 `Observation.code` 之問題碼 | **P0** | 術語／建模／對外一致性 | S–M（1–1.5 人日） | 須先完成 §4.1 之 LOINC 查證 | 📋 **評估（v0.7.2）**：委員技術判斷**成立**，且有更強實證——已發佈之 `obs-betelnut-never` 其 `code`＝`698188003`（Chews betel quid）而 `value`＝`0-never`（從未嚼食），**同一資源自相矛盾**，以 `code` 檢索會撈出從未嚼檳者（偽陽性；`err=0` 仍成立因驗證器不檢查語意相容性）。惟**處置方案不同意**——回復綁定上游會推翻 C-0 之解耦成果。建議第三方案：`.code` 改自訂狀態問題碼、`.value` 四碼不動、`698188003` 改列為肯定式狀態之 SNOMED 對應。並更正 JOB-29 §D.3a 之推論。⚠️ 屬 **Level 1 breaking change**（觸點 13 處），且**與 JOB-30 §3.4 送簽確認單時序衝突，確認單未處置前不得送出**。**待裁示後實作** |
| [JOB-33](JOB-33-clean-release-for-trial.md) | 對外試用版乾淨化：內部痕跡、警語收斂、未決事項外移、發布模式 | **P0** | 發布治理／呈現／建置設定 | M–L（3–4 人日） | 步驟 3 須 PI 先裁示 canonical | 📋 **評估與計畫（v0.7.3）**：實測（v0.6.1，須重測）**21 頁／122 處** `JOB-n` 外洩，`Description` 內含者隨四種渲染變體擴散。**關鍵**：移除 continuous build 標記不是呈現問題，前提是 canonical——`qa.txt` **2321 筆斷鏈中 2320 筆為同一目標** `twcore.../history.html`，而 `history.html` 本站已生成，**斷的是 canonical 前綴**（v0.7.1 複驗數字不變）；改為 github.io 命名空間可使 banner 正常且**斷鏈歸零**。警語分內部備註型（刪）與規範性揭露（收斂為首頁六條〈使用前須知〉，**不得刪**）。分三步：v0.7.4／v0.8.0／v0.9.0。**待裁示後實作** |
| [JOB-34](JOB-34-composition-traversal-and-imagingstudy-bindings.md) | 17 筆結構性 WARNING：10 筆 Composition 走訪不到 ＋ 7 筆 ImagingStudy 值集解析不到 | P1 | 範例正確性／文件語意／上游術語 | S（診斷已完成）＋ M（實作） | 無（與 JOB-32 步驟 2 皆動 `03-social-history.fsh`，需避免互相覆蓋） | 🔍 **診斷完成（v0.8.3）**：以參照圖自 Composition 做前向可達性走訪（R4 §3.3.1）實測——**七個 document Bundle 中六個完全乾淨，問題全在 UC-003**（19 個 entry 僅 8 個可達）。故**不是 Bundle type 用錯、也不是建模缺陷，是 `composition-uc003` 的 section 沒把內容歸檔**。⚠️ 其中兩筆屬範例語意問題而非清理工作：UC-003 同時帶一般與特殊兩個 Encounter 卻只引用一般那個；`example-nurse` 無任何資源參照（孤兒），應移除而非硬塞進 section。**差額已查明**：本診斷 11 筆 vs `qa.txt` 10 筆，差在 `example-nurse`——它確實走訪不到，只是 IG Publisher 未為其發警告（**機制未確立，標為未驗證**）；11 不改成 10，否則會把唯一該刪的資源藏起來。ImagingStudy 那 7 筆定性為**上游條件且永不消失**：實測 canonical 原值為 `…RadLex_Playbook.aspx` 與 `…sect_B.5.html#table_B.5-1`，**兩者皆為 HTML 說明頁網址而非 ValueSet 資源**，任何術語伺服器都解不開；故維持現狀＋載明為唯一正確選項，**不得以 `ignoreWarnings.txt` 抑制**。**步驟 2 待裁示** |
