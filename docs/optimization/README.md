...
| [JOB-34](JOB-34-composition-traversal-and-imagingstudy-bindings.md) | 17 筆結構性 WARNING：10 筆 Composition 走訪不到 ＋ 7 筆 ImagingStudy 值集解析不到 | P1 | 範例正確性／文件語意／上游術語 | S（診斷已完成）＋ M（實作） | 無（與 JOB-32 步驟 2 皆動 `03-social-history.fsh`，需避免互相覆蓋） | 🔍 **診斷（v0.8.3）**：以參照圖自 Composition 做前向可達性走訪（R4 §3.3.1）實測——**七個 document Bundle 中六個完全乾淨，問題全在 UC-003**（19 個 entry 僅 8 個可達）。故**不是 Bundle type 用錯、也不是建模缺陷，是 `composition-uc003` 的 section 沒把內容歸檔**。⚠️ 其中兩筆屬範例語意問題而非清理工作：UC-003 同時帶一般與特殊兩個 Encounter 卻只引用一般那個；`example-nurse` 無任何資源參照（孤兒），應移除而非硬塞進 section。⚠️ **未解差額**：本診斷算出 11 筆、`qa.txt` 為 10 筆，成因未查明——已排除漏抓參照與 Provenance 豁免；**兩個數字都保留，不取其一**（比照 CLAUDE.md §2.4「21 列 vs 20 碼」）。ImagingStudy 那 7 筆定性為**上游條件**：本 IG 之 profile 只加了 `subject only`，3 筆綁定出自 R4 核心 ImagingStudy 自身，值集屬 DICOM／RadLex 體系不隨核心套件散布，建議維持現狀＋載明，**不得以 `ignoreWarnings.txt` 抑制**。**待步驟 1 取得 qa.txt 逐筆原文後方可實作** |
