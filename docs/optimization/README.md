...
| [JOB-19](JOB-19-ucum-snomed-asset-audit.md) | 術語資產稽核與閘門化（UCUM 單位、SNOMED 對映） | **P0** | 術語正確性／工程 | M（3–5 人日） | JOB-17／18 | 🟡 **執行中**（**線 A 已執行**：`snomed-loinc-mappings.csv` 逐筆覆核，視力 `79880-1`→`98497-1`、WHR `73708-3`→無碼哨符，33 列 `snomed_status` 全改 `待覆核`；新增 `scripts/check-asset-consistency.js` 納入 `check:assets`＋負向測試。**線 B（UCUM 四態稽核＋`UCUM mismatch` 閘門）待另一 PR 以 CI 為 `$lookup` oracle 執行**） |
