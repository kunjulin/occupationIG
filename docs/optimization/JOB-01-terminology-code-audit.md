...
**已產出可逕行發文之確認單**：[`CLINICAL-QUERY-JOB-01.md`](CLINICAL-QUERY-JOB-01.md)
——8 題、23 碼，每題含本 IG 現標／LOINC 官方定義／勾選選項／該答案的後果，
末附一頁式回覆彙整表。第 1–7 題送檢驗醫學部、第 8 題送職業醫學科。

確認單刻意**不預設替代代碼**：只問「貴部實際上怎麼報」，換不換碼、換成哪一碼
由本專案依答覆再以 `$lookup` 查證。理由見 §2.2——未經查證的候選碼曾在本 JOB
執行過程中出過錯（`1916-6` 實為 AST/ALT 比值，非 AST with P5P）。

### 附帶發現：下載資產與 IG 本體不一致

`input/assets/snomed-loinc-mappings.csv` 之 eGFR（CKD-EPI）一列標 `88293-6`
且註記 `VERIFIED`，但 IG 本體實收 `98979-8`（CKD-EPI 2021）。該 CSV 為
下載專區公開檔案，兩者不一致。`88293-6` 未經本次 `$lookup` 查證，
故**不逕行更動**，列為確認單附錄 A 一併處理。
