# Health Management - 臺灣勞工健康檢查交換實作指引 (Taiwan Labor Health Examination Exchange FHIR IG, TWHA IG) v0.1.0
...
> 範例：[第四級管理判定 obs-health-mgmt-level](Observation-obs-health-mgmt-level.md)（噪音作業特殊健檢後之判定， 完整情境見 [UC-003 封包](Bundle-UC-003.md)）。
...
> 範例：[適性配工計畫 example-careplan-fitness](CarePlan-example-careplan-fitness.md)——變更工作場所＋縮短工時， 對應前述第四級管理判定。
...
> 範例：[追蹤檢查要求 example-servicerequest-followup](ServiceRequest-example-servicerequest-followup.md)——三個月後純音聽力複檢。
