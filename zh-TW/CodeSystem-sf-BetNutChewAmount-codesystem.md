# 【本地 stub】每日嚼檳榔量代碼系統（非權威定義） - 臺灣勞工健康檢查交換實作指引 (Taiwan Labor Health Examination Exchange FHIR IG, TWHA IG) v0.2.5

## CodeSystem: 【本地 stub】每日嚼檳榔量代碼系統（非權威定義） (實驗性) 

 
Betel Nut Chewing Amount Code System 

下列值集之定義引用本代碼系統：

* [SFBetNutChewAmountValueSet](ValueSet-sf-BetNutChewAmount-valueset.md)

-------

 [上表之說明](http://build.fhir.org/ig/FHIR/ig-guidance/readingIgs.html#terminology). 



## Resource Content

```json
{
  "resourceType" : "CodeSystem",
  "id" : "sf-BetNutChewAmount-codesystem",
  "url" : "https://hapi.fhir.tw/fhir/CodeSystem/sf-BetNutChewAmount-codesystem",
  "version" : "0.2.5",
  "name" : "sf-BetNutChewAmount-codesystem",
  "title" : "【本地 stub】每日嚼檳榔量代碼系統（非權威定義）",
  "status" : "draft",
  "experimental" : true,
  "date" : "2026-08-17T06:57:01+00:00",
  "publisher" : "衛生福利部次世代數位醫療平臺專案辦公室 & 長庚醫療財團法人長庚紀念醫院",
  "contact" : [{
    "name" : "衛生福利部次世代數位醫療平臺專案辦公室 & 長庚醫療財團法人長庚紀念醫院",
    "telecom" : [{
      "system" : "url",
      "value" : "https://twcore.mohw.gov.tw/twregistry/"
    }]
  },
  {
    "name" : "衛生福利部次世代數位醫療平臺專案辦公室",
    "telecom" : [{
      "system" : "url",
      "value" : "https://twcore.mohw.gov.tw/twregistry/"
    }]
  }],
  "description" : "Betel Nut Chewing Amount Code System",
  "copyright" : "權威定義來源為臺灣癌症登記短表 IG（TWCR_SF）。本 IG 僅為建置與驗證所需之局部複本（content = fragment），非權威定義；實作端應以 TWCR_SF 官方定義為準。",
  "content" : "fragment",
  "concept" : [{
    "code" : "00",
    "display" : "無嚼檳榔"
  },
  {
    "code" : "01",
    "display" : "每日1顆"
  },
  {
    "code" : "02",
    "display" : "每日2顆"
  },
  {
    "code" : "03",
    "display" : "每日3顆"
  },
  {
    "code" : "04",
    "display" : "每日4顆"
  },
  {
    "code" : "05",
    "display" : "每日5顆"
  },
  {
    "code" : "06",
    "display" : "每日6顆"
  },
  {
    "code" : "07",
    "display" : "每日7顆"
  },
  {
    "code" : "08",
    "display" : "每日8顆"
  },
  {
    "code" : "09",
    "display" : "每日9顆"
  },
  {
    "code" : "10",
    "display" : "每日10顆"
  },
  {
    "code" : "11",
    "display" : "每日11顆"
  },
  {
    "code" : "12",
    "display" : "每日12顆"
  },
  {
    "code" : "13",
    "display" : "每日13顆"
  },
  {
    "code" : "14",
    "display" : "每日14顆"
  },
  {
    "code" : "15",
    "display" : "每日15顆"
  },
  {
    "code" : "16",
    "display" : "每日16顆"
  },
  {
    "code" : "17",
    "display" : "每日17顆"
  },
  {
    "code" : "18",
    "display" : "每日18顆"
  },
  {
    "code" : "19",
    "display" : "每日19顆"
  },
  {
    "code" : "20",
    "display" : "每日20顆"
  },
  {
    "code" : "21",
    "display" : "每日21顆"
  },
  {
    "code" : "22",
    "display" : "每日22顆"
  },
  {
    "code" : "23",
    "display" : "每日23顆"
  },
  {
    "code" : "24",
    "display" : "每日24顆"
  },
  {
    "code" : "25",
    "display" : "每日25顆"
  },
  {
    "code" : "26",
    "display" : "每日26顆"
  },
  {
    "code" : "27",
    "display" : "每日27顆"
  },
  {
    "code" : "28",
    "display" : "每日28顆"
  },
  {
    "code" : "29",
    "display" : "每日29顆"
  },
  {
    "code" : "30",
    "display" : "每日30顆"
  },
  {
    "code" : "31",
    "display" : "每日31顆"
  },
  {
    "code" : "32",
    "display" : "每日32顆"
  },
  {
    "code" : "33",
    "display" : "每日33顆"
  },
  {
    "code" : "34",
    "display" : "每日34顆"
  },
  {
    "code" : "35",
    "display" : "每日35顆"
  },
  {
    "code" : "36",
    "display" : "每日36顆"
  },
  {
    "code" : "37",
    "display" : "每日37顆"
  },
  {
    "code" : "38",
    "display" : "每日38顆"
  },
  {
    "code" : "39",
    "display" : "每日39顆"
  },
  {
    "code" : "40",
    "display" : "每日40顆"
  },
  {
    "code" : "41",
    "display" : "每日41顆"
  },
  {
    "code" : "42",
    "display" : "每日42顆"
  },
  {
    "code" : "43",
    "display" : "每日43顆"
  },
  {
    "code" : "44",
    "display" : "每日44顆"
  },
  {
    "code" : "45",
    "display" : "每日45顆"
  },
  {
    "code" : "46",
    "display" : "每日46顆"
  },
  {
    "code" : "47",
    "display" : "每日47顆"
  },
  {
    "code" : "48",
    "display" : "每日48顆"
  },
  {
    "code" : "49",
    "display" : "每日49顆"
  },
  {
    "code" : "50",
    "display" : "每日50顆"
  },
  {
    "code" : "51",
    "display" : "每日51顆"
  },
  {
    "code" : "52",
    "display" : "每日52顆"
  },
  {
    "code" : "53",
    "display" : "每日53顆"
  },
  {
    "code" : "54",
    "display" : "每日54顆"
  },
  {
    "code" : "55",
    "display" : "每日55顆"
  },
  {
    "code" : "56",
    "display" : "每日56顆"
  },
  {
    "code" : "57",
    "display" : "每日57顆"
  },
  {
    "code" : "58",
    "display" : "每日58顆"
  },
  {
    "code" : "59",
    "display" : "每日59顆"
  },
  {
    "code" : "60",
    "display" : "每日60顆"
  },
  {
    "code" : "61",
    "display" : "每日61顆"
  },
  {
    "code" : "62",
    "display" : "每日62顆"
  },
  {
    "code" : "63",
    "display" : "每日63顆"
  },
  {
    "code" : "64",
    "display" : "每日64顆"
  },
  {
    "code" : "65",
    "display" : "每日65顆"
  },
  {
    "code" : "66",
    "display" : "每日66顆"
  },
  {
    "code" : "67",
    "display" : "每日67顆"
  },
  {
    "code" : "68",
    "display" : "每日68顆"
  },
  {
    "code" : "69",
    "display" : "每日69顆"
  },
  {
    "code" : "70",
    "display" : "每日70顆"
  },
  {
    "code" : "71",
    "display" : "每日71顆"
  },
  {
    "code" : "72",
    "display" : "每日72顆"
  },
  {
    "code" : "73",
    "display" : "每日73顆"
  },
  {
    "code" : "74",
    "display" : "每日74顆"
  },
  {
    "code" : "75",
    "display" : "每日75顆"
  },
  {
    "code" : "76",
    "display" : "每日76顆"
  },
  {
    "code" : "77",
    "display" : "每日77顆"
  },
  {
    "code" : "78",
    "display" : "每日78顆"
  },
  {
    "code" : "79",
    "display" : "每日79顆"
  },
  {
    "code" : "80",
    "display" : "每日80顆"
  },
  {
    "code" : "81",
    "display" : "每日81顆"
  },
  {
    "code" : "82",
    "display" : "每日82顆"
  },
  {
    "code" : "83",
    "display" : "每日83顆"
  },
  {
    "code" : "84",
    "display" : "每日84顆"
  },
  {
    "code" : "85",
    "display" : "每日85顆"
  },
  {
    "code" : "86",
    "display" : "每日86顆"
  },
  {
    "code" : "87",
    "display" : "每日87顆"
  },
  {
    "code" : "88",
    "display" : "每日88顆"
  },
  {
    "code" : "89",
    "display" : "每日89顆"
  },
  {
    "code" : "90",
    "display" : "每日≧90顆"
  },
  {
    "code" : "91",
    "display" : "偶爾嚼(無規律或無定量)"
  },
  {
    "code" : "98",
    "display" : "有嚼，但量不詳"
  },
  {
    "code" : "99",
    "display" : "病歷未記載或嚼檳榔狀態完全不詳者"
  }]
}

```
