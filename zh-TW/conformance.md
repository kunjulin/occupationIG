# 遵從性與依賴 - 臺灣勞工健康檢查交換實作指引 (Taiwan Labor Health Examination Exchange FHIR IG, TWHA IG) v0.10.3
...
下表逐列取自**主管機關上傳欄位原案**（TWHA IG 完整編碼附件 **v7.7**〈Core 主管機關最小集(21)〉； 原案內容與 v7.6 同，v7.7 更新之嚼檳三列敘述已反映於下表）， 以**健保醫令代碼**為鍵值；「本指引承載」欄說明各列於本指引中實際落在哪個元素。 acceptable 變異碼經 [ConceptMap](ConceptMap-TWHealthCheckLaboratoryMap.md) 歸一，不另計列。
...
> 📌 **v0.10.1 新增之 acceptable 變異碼（不影響上表之 21 列，亦不影響 preferred 主碼）** ——2026-08-22 委員決議，經 `tx.fhir.org` `$lookup` 查證後收錄：
...
⚠️ **`2887-8`／`20454-5` 標 `relatedto` 而非 `wider`，是刻意的。** 本指引於 [術語頁 §3.1.1](terminology.md) 允許尿蛋白定性之跨 Scale 綁定，惟該決議屬**值集層**； ConceptMap 之 `wider`／`narrower` 是**歸一層**對語意包含關係之斷言， 而 `PrThr` 與 `MCnc` 之間沒有包含關係。兩層不得互相覆蓋。⚠️ **兩列之 Preferred 均未變**（腰圍仍為 `8280-0`、三酸甘油酯仍為 `2571-8`）， 上表所列之 21 列與各列主碼**一碼未動**。變的只是各該列可被接受並歸一之變異碼範圍。 腰圍另有三個**量測 protocol 碼**（`56086-2` PhenX、`56114-2` NHANES、`56115-9` NCFS） 僅登錄於 ConceptMap 供歸一，**刻意不納入任何值集**——它們是 protocol 而非量測碼， 收進值集會使實作端誤以為可作 `Observation.code`。
...
> ⚠️ **`VS-CoreUploadSet` 不等於本表——引用時務必分清。** [VS-CoreUploadSet](ValueSet-VS-CoreUploadSet.md) 是**跨值集之群組**， 其展開結果與本表之 21 列**計的不是同一件事**：後者是主管機關原案之**欄位列數**， 前者是 FHIR 值集之**成員代碼數**。現存差異為兩處：
...
原有第三處差異「＋ BMI `39156-5`」 → **v0.10.0 起已由值集明文排除** （`exclude LNC#39156-5`），不再靠註解宣告，故該值集之展開結果**不再含 BMI**。 原以註解宣告之作法**攔不住機器**：任何以值集展開做涵蓋核對之程式， 看到的仍是含 BMI 的清單。核算式隨之改為：**preferred 主碼數 ＋ 2（嚼檳量、嚼食持續期間——本指引以 `component` 承載，未另立代碼）= 21**🔴 **更正（v0.10.1）——這條算式先前掛在錯的名詞上。** v0.5.0–v0.10.0 期間本頁寫作「**展開碼數** ＋ 2 ＝ 21」，並在同一段把「展開碼數」定義為 「FHIR 值集之成員代碼數」。**兩者不能同時成立**：本群組值集之 `$expand` 會一併帶出 各項目之 acceptable 變異碼（`VS-CoreDataset` 一個值集之碼數本身就已超過 21）， 成員代碼數遠大於 21，`成員代碼數 ＋ 2` 不會等於 21。 算式其實是對的，只是它算的是「**每一列之 preferred 主碼**」—— 本指引 v0.5.0 首次量得該數時之定義即為此（檢驗子集 10 ＋ 生理量測 6 ＋ 社會史 4 ＝ 20， v0.10.0 排除 BMI 後為 19）。**錯的是名詞不是數字**，故本次只更正名詞與定義。⚠️ **兩個數字都不在本頁載死，一律以該次建置之實測為準：**
...
理由見 [`CLAUDE.md`](https://github.com/kunjulin/occupationIG/blob/main/CLAUDE.md) §4 「沿用文件所載之數字」一列——本專案已因轉抄數字而出錯數次，本次則是**因為沿用了一個 沒被驗算過的名詞**。需要現值時請讀該次建置之值集展開，或 `qa-baseline.json` 之相關類別。**「21 列」與上開任一個碼數計的仍不是同一件事**，這一點未因本次調整而改變： 21 列是主管機關原案之**欄位列數**，兩個碼數都是 FHIR 側的東西。
...
**上開三項之依據各不相同，不得合併表述**——三者均**無須委託單位或主管機關另行核定**， 但理由不是同一個：
* **①（BMI 不納入）＝ 事實陳述。** 主管機關原案 21 列中本來就沒有 BMI， 本次只是讓值集之機器可讀結果回到原案。**不涉任何會議或機關之新增認定。**
* **②（第 11 列語意）＝ 本案專家會議之技術決議**（2026-07-17 七、臨時動議）， 屬**受託團隊職權範圍**。三項之中，唯有此項需要人的判斷。
* **③（`{quid}/d`）＝ 術語技術正確性。** UCUM 之 annotation 僅接受可列印 ASCII， `{個}` 非合法 UCUM，**任何符合規格之實作均無法採用**，無裁量空間。
🔴 **更正一（v0.10.0）**：本表於 v0.6.0–v0.9.3 期間記為「**已獲主管機關答覆**」， **該敘述不實——國民健康署迄未就此作成函復**。 「經主管機關確認」是規範文件裡效力最強的措辭之一，誤用會使該署日後讀到與事實不符。 全庫同型敘述經逐處更正。🔴 **更正二（v0.10.1）**：v0.10.0 改為「上開三項**均為**本案專家會議之技術決議」， **仍屬過度歸因**——會議紀錄之七、臨時動議**只涵蓋②**，①與③並未在該次會議作成決議。 更嚴重的是，該句就印在上方那張已逐項分開之表格正下方，**同一頁自相矛盾**。 ⚠️ 把①③掛到會議決議上其實是**削弱**它們：「原案未列」與「UCUM 不接受非 ASCII」 是事實與規格，任何人都可逕行覆核；一旦寫成合議結論，反而降格成須舉證之事項。 這是同一類錯誤第三次出現（先誤記為主管機關答覆、再誤記為委託單位指示、此次誤記為 會議決議均涵蓋）——**歸因要逐項核對來源文件，不可整批套用同一個出處**。⚠️ **仍屬國民健康署權責者**：21 列清單本身之**法定效力**、**正式公告內容與生效時點**。
...
* **第 11 列**（30907X-3）語意為**嚼食持續期間**，承接自第 10 列移出之年數， **且不論現嚼或已戒均應填列**。
✅ **戒除資訊不因本更正而遺失**：原案 r4 所指之戒檳月數並未捨棄，改由 `component[cessationDuration]`（戒除期間）與 `component[cessationDate]`（戒除日期） 承載，**不佔最小上傳集之列位**。📮 **已建請主管機關更正原案文字**（見未決事項 **M-12**，對應專家會議文件三之 M-7）： 報告序號 2 之 r4 改為「（填寫平均每日嚼食顆數）」；報告序號 3 之 r4 改為 「（填寫自開始嚼食之持續期間，以月填列）」；註 2 之填列條件改為不論狀態 1、2、3 均填列， 舉例改為「共嚼食 2 年 1 個月，請填 25」。**上開更正不改變欄位數，仍為 21 列。** ⚠️ 該建議之採納與否由該署決定；在其正式公告前，本指引依專家會議決議辦理， **並於正式公告後複核對齊**。

> ⚠️ **定案的是這三項，不是 M-5。** 仍待國民健康署者為： ① **21 列清單本身之法定效力**；② **正式公告內容與生效時點**。 兩者屬該署行政公告權責，非委託單位可代為認定，故 **M-5 維持未解決**，嚼檳系列之 `experimental` 旗標與 Level 1 之成熟度亦一律不變更。

> 📌 **②之說明**：原案欄名「嚼檳月數」與吸菸列之「戒菸月數」形式對稱， 易被讀成「戒檳月數」；兩者方向相反（一個是還在嚼、一個是已經戒）， 對映錯誤會使資料語意顛倒，故特此載明。 單位放寬為 `a` 或 `mo` 而**不做強制換算**——把「嚼 18 個月」寫成「1.5 年」 同樣會偽造精度。`component[cessationDuration]`（戒除期間）仍保留， 但**不屬最小上傳集之欄位**。`CS-BetelNutComponent` 之代碼 id 保留 `duration-years`（含 "years" 字樣）： 該碼已於 v0.4.0 發佈，改名屬破壞性變更。**真實語意以顯示名與定義為準， 不以代碼 id 為準**——代碼 id 只是識別字。
