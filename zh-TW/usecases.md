# Usecases - 臺灣勞工健康檢查交換實作指引 (Taiwan Labor Health Examination Exchange FHIR IG, TWHA IG) v0.1.0
...
* **情境**：勞工於急診就醫時，急診醫師需要快速取得其職業暴露史與最近健檢重點， 以判斷症狀是否與職業暴露相關。
* **要點**： 
...
* **對應範例**：[UC-007 職業健康急診友善摘要封包範例](Bundle-UC-007.md)。
...
* **要點**： 
* `TWHA-Bundle-Transaction`：每個 entry 具 `request.method`／`request.url`， 內部參照以 `urn:uuid` 表達；
