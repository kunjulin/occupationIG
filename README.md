...
### v0.6.0（2026-08-20）三項差異之落地 ＋ 勞工區塊標 `draft`（JOB-30 §7.8）

> 🔴 **標題更正（v0.10.0）**：原標題為「**主管機關答覆**之落地」，該用語不實，
> 已改為中性之「三項差異之落地」。理由見本節下方之更正框。
...
> 🔴 **更正（v0.10.0）**：本節原記「已獲主管機關答覆」，**該敘述不實**——
> 國民健康署從未就此三項答覆。實際依據為**委託單位（工業技術研究院）之指示**，
> 該單位並於 2026-08-21 表明「皆非國健署需要確認事項」。
> 依本 repo 慣例保留原文並加註更正，不逕行改寫歷史；現行敘述見 v0.10.0 條目與
> [conformance.md §7.2](input/pagecontent/conformance.md)。

**三項差異之依據（更正後）**：委託單位指示，非主管機關答覆——

| # | 事項 | 指示內容 | 本版（v0.6.0）之處置 |
|:--|:--|:--|:--|
| ① | BMI 是否納入 21 列 | **不納入** | 維持現況；`39156-5` 供生理量測用，不屬最小上傳集。⚠️ **v0.10.0 起改為值集明文 `exclude`** |
...
- ⚠️ **上開指示為口頭轉述**：依既定立場，**取得可引用之書面依據前，M-5 狀態、
