...
| 項目 | 狀態 |
|:--|:--|
| **版控**（`sushi-config.yaml`） | **v0.10.0**（尚未發佈） |
| **線上站台** | **v0.9.3**，gh-pages `24bdf5b1`（sourceCommit `39a3dcc7`，2026-08-21 16:12Z），已逐項複驗 |
...
| **② Core upload set** | 主管機關（國健署）最小共通上傳集，**16 主項／21 列**；⚠️ `VS-CoreUploadSet` 之展開碼數與 21 列**計的不是同一件事**（見下）。**展開碼數不在此載死，以該次建置實測為準** | `VS-CoreUploadSet` |
...
>
> 核算（v0.10.0 起）：**展開碼數 ＋ 2 = 21**。⚠️ **展開碼數不在此載死，以該次建置實測為準。**
...
>
> **三項實質差異已依本案專家會議決議定案**（v0.10.0，場次與日期：`[專家會議場次與日期待補]`）：
> ① BMI **不納入**（回到原案，值集已明文 `exclude`）；
> ② 第 11 列「嚼檳月數」＝**嚼食持續期間**（非戒檳月數，兩者語意相反）；
> ③ 嚼檳量單位採 `{quid}/d`（`{個}` 非合法 UCUM——annotation 僅接受可列印 ASCII）。
>
> 🔴 **這三項的依據是「本案專家會議之技術決議」，屬受託團隊職權範圍，
> 無須委託單位或主管機關另行核定；更不是「主管機關答覆」。**
> v0.6.0–v0.9.3 期間全庫有 5 處記為「經主管機關答覆確認」，**該敘述不實**——
> 國健署從未就此答覆，已於 v0.10.0 逐處更正。
