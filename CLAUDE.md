...
| 送國健署確認單 | `docs/drafts/HPA-CONFIRMATION-JOB-30.md` — **可送出**（v0.10.0 已瘦身：第 1～3 項改列備查，逐項簽核僅餘第 4～5 項）；由 PI 核閱後以正式函文發出，**本團隊不逕行對外發送** |
...
> | 差異 | 說明 |
> |:--|:--|
> | ~~群組值集多了 BMI `39156-5`~~ | ~~不是 Core 列；係 `VS-TWHAVitalSigns` 之成員被群組一併帶入~~ → **v0.10.0 已由值集明文 `exclude`**，此差異消滅。依委託單位 2026-08-21 指示不納入（**回到原案**，非新增主張）。`VS-TWHAVitalSigns` 不動——BMI 仍是有效之生理量測項目 |
...
> 屬該署行政公告權責，非委託單位可代為認定，**故 M-5 維持未解決**。
...
* 國健署最小上傳集之正式公告版本（**M-5**）。逐列組成已由原案 v7.6 查明（§2.4）；
  **三項實質差異已依委託單位 2026-08-21 指示定案（v0.10.0）**，
  ...
  **狀態（2026-08-22）：可送出，已交 PI 核閱；v0.10.0 已瘦身**——第 1～3 項改列備查，
  逐項簽核僅餘 4-A（21 列法定效力）、4-B（逐列對照）與第 5 項（生效時點）。
