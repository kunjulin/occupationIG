...
| 項目 | 狀態 |
|:--|:--|
| **版控**（`sushi-config.yaml`） | **v0.10.3** |
| **線上站台** | **v0.10.3**，gh-pages `23f485b7`（sourceCommit `8223e3cc`，2026-08-22 17:17Z），已逐項複驗 |
...
| **對外三份文件** | 完整編碼附件 **v7.7**、建議修訂案 **v2.2**、專家共識討論會會議紀錄（2026-07-17）。**待更正項目：0**（2026-08-22 覆核）——v7.7 現載 49 組、`wider` 14／`narrower` 7／`relatedto` 28，與線上 v0.10.3 逐組相符；#38 已為 `relatedto`／「⚠ 不可」，#46 備註已改寫。⚠️ **本列曾自 v0.10.1 起連續三輪列出已完成之項目**——文件端交付後未回頭清帳，使已結案事項被重複列管。**更新 IG 端狀態時，文件端這一列要一併回頭確認，不是只往上加。** |
...
| 送國健署確認單 | `docs/drafts/HPA-CONFIRMATION-JOB-30.md` — **可送出**（v0.10.0 已瘦身：第 1～3 項改列備查，逐項簽核為第 4～6 項）；由 PI 核閱後以正式函文發出，**本團隊不逕行對外發送** |
| canonical 命名空間 | 仍為 provisional，**本批未動**。工研院指示有兩種解讀，已產出備忘 `docs/drafts/CANONICAL-NAMESPACE-MEMO.md` 待其裁示 |
| M-5 狀態／嚼檳系列 `experimental`／Level 1 成熟度 | **仍全部擋下**，須取得可引用之**書面**依據（口頭／轉述不足）。⚠️ 三項實質差異之定案**不解除**此擋 |
...
| **② Core upload set** | 主管機關（國健署）最小共通上傳集，**16 主項／21 列**；⚠️ `VS-CoreUploadSet` 之 **preferred 主碼數**與 21 列**計的不是同一件事**（見下；`$expand` 之成員代碼數又是第三個數，含所有 acceptable 變異碼）。**兩個碼數都不在此載死，以該次建置實測為準** | `VS-CoreUploadSet` |
...
> ⚠️ **更新（v0.5.0，JOB-30）：21 列是對的，`VS-CoreUploadSet` 之 preferred 主碼為 20 碼
> 也是對的——兩者計的不是同一件事。** 差異已依主管機關上傳欄位原案
> （TWHA IG 完整編碼附件 v7.7〈Core 主管機關最小集(21)〉，內容同 v7.6）逐列查明，共三處：
>
> | 差異 | 說明 |
> |:--|:--|
> | ~~群組值集多了 BMI `39156-5`~~ | ~~不是 Core 列；係 `VS-TWHAVitalSigns` 之成員被群組一併帶入~~ → **v0.10.0 已由值集明文 `exclude`**，此差異消滅。因**原案未列 BMI**而不納入（回到原案，非新增主張）。`VS-TWHAVitalSigns` 不動——BMI 仍是有效之生理量測項目 |
> | 群組值集少了嚼檳量（`30907X-2`） | 原案為獨立一列，本 IG 以 `component[amount]` 承載，非獨立 `Observation.code` |
> | 群組值集少了嚼檳月數（`30907X-3`） | 同上，以 `component[durationYears]` 承載 |
>
> 核算（v0.10.0 起）：**preferred 主碼數 ＋ 2 = 21**。⚠️ **碼數不在此載死，以該次建置實測為準。**
>
> 🔴 **更正（v0.10.1）**：本行原寫「**展開碼數** ＋ 2 = 21」，名詞用錯。
> 「展開」在 FHIR 指 `$expand`，其結果**含所有 acceptable 變異碼**
> （光 `VS-CoreDataset` 一個值集之碼數就已超過 21），**成員代碼數 ＋ 2 不會等於 21**。
> 算式本身沒錯，它算的是「每列之 **preferred 主碼**」——JOB-30 原始量測即為
> 檢驗子集 10 ＋ 生理量測 6 ＋ 社會史 4 ＝ 20（v0.10.0 排除 BMI 後為 19）。
> **兩個碼數都要以實測為準，但引用時務必說清楚是哪一個**——這次出錯不是抄錯數字，
> 是沿用了一個沒被驗算過的名詞，而該名詞已被複製到對外文件。
> 逐列對照見 [conformance.md §7.2](input/pagecontent/conformance.md)。
...
>
> **三項實質差異均已定案**（v0.10.0）。⚠️ **三者之依據各不相同，不得合併表述**——
> 均無須委託單位或主管機關另行核定，但理由不是同一個：
>
> | # | 定案內容 | 依據 | 性質 |
> |:--|:--|:--|:--|
> | ① | BMI **不納入**（回到原案，值集已明文 `exclude`） | **原案未列 BMI** | **事實陳述**，不涉任何一方之新增認定 |
> | ② | 第 11 列「嚼檳月數」＝**嚼食持續期間**（非戒檳月數，兩者語意相反） | 本案專家共識會議（臺灣勞工健康檢查核心資料集與 FHIR IG 專家共識討論會，**2026-07-17**）**七、臨時動議**之決議；共同主持人曹又中主任 2026-07-31 回信補充說明 | **技術決議**，屬受託團隊職權範圍 |
> | ③ | 嚼檳量單位採 `{quid}/d` | UCUM 之 annotation 僅接受可列印 ASCII，`{個}` 非合法 UCUM | **術語技術正確性**，無裁量空間 |
>
> 🔴 **同一類錯誤已出現三次，記在這裡是為了不要有第四次：**
> v0.6.0–v0.9.3 全庫有 5 處記為「經主管機關答覆確認」（**不實**，國健署從未答覆）；
> PR #45 初版改記為「依委託單位（工研院）指示」（**亦非**）；
> v0.10.0 改為「三項**均為**專家會議決議」（**過度歸因**——會議紀錄只涵蓋②），
> 且該句就印在 `conformance.md` 已逐項分開之表格正下方，同一頁自相矛盾，v0.10.1 更正。
> ⚠️ 把①③掛到會議上其實是**削弱**它們：事實與規格任何人都可逕行覆核，
> 寫成合議結論反而降格為須舉證之事項。
> **歸因要逐項回去核對來源文件，不可整批套用同一個出處。**
> **「經主管機關確認」是規範文件裡效力最強的措辭之一，不得用於未發生的認定。**
>
> ⚠️ **仍待國健署者為兩項**（見確認單第 4 項之 4-A／4-B）：
> ⓐ **21 列清單本身之法定效力**；ⓑ **正式公告內容與生效時點**。
> 屬該署行政公告權責，非受託團隊或委託單位可代為認定，**故 M-5 維持未解決**。
> ⚠️ 另就**原案文字之更正**已具體建議該署，見未決事項 **M-12**（對應專家會議文件三之 M-7）。
...
| 沿用文件所載之數字（如「21 列」「24 處錨點」） | **自原始碼逐項數出來，並回頭核對權威來源**。JOB-30 實測：`VS-CoreUploadSet` 之 **preferred 主碼 20 碼**（非 21；⚠️ v0.10.1 更正名詞，原記為「展開 20 碼」——`$expand` 之成員代碼數含 acceptable 變異碼，遠不止此數）、具名錨點 **23 處**（非 24）。⚠️ 但「20 碼」**不代表 21 列是錯的**——回查原案後查明兩者計的不是同一件事（見 §2.4）。**數不符時先找權威來源，不要急著改文件數字，也不要急著補足差額。** |
...
| 結案回報之涵蓋範圍大於實際完成範圍 | 與「閘門是綠的，但範圍不對」同型，只是發生在**回報**上。實例：v0.10.3 那輪宣告「工作全部結束」，而該輪指定事項中之 `downloads.md` 硬編版號未執行——其餘各項都做了且都通過閘門，所以自我檢查一路是綠的，**因為檢查清單裡沒有「逐條對回該輪的指定事項」這一步**。作法：結案前把該輪指定事項逐條列出、逐條標記已做／未做／不適用，再宣告完成；「其他都做完了」不足以支撐「全部結束」。 |
| 已完成之事項留在待辦列上重複列管 | 待辦列只往上加、不回頭清帳，會使已結案事項看起來還沒做。實例：§1.1「對外三份文件」自 v0.10.1 起連續三輪列出文件端早已完成之項目。⚠️ 這與漏做相反但成本相同——**列表失真的方向不影響它失去可信度**。更新任一列時，同表其他列要一併回頭確認。 |
...
* 正式 canonical namespace 之核定機關與命名（現為 provisional）。
  ⚠️ 工研院 2026-08-21 表示「建議由工研院調整即可」，惟該句有兩種解讀
  （代為向命名空間擁有者申請／指定其自有位址），且**工研院不是 `twcore.mohw.gov.tw`
  之擁有者**（該命名空間屬衛福部次世代數位醫療平臺專案辦公室），**無權逕行授予**。
  **未取得指定位址前不得變更 canonical**——變更即為破壞性變更，全部資源 URL 改變。
  三個選項與代價見 `docs/drafts/CANONICAL-NAMESPACE-MEMO.md`。
* 國健署最小上傳集之正式公告版本（**M-5**）。逐列組成已由原案查明（§2.4；現行對照見完整編碼附件 v7.7，原案內容同 v7.6）；
  **三項實質差異已依本案專家會議決議定案（v0.10.0）**，
  仍待該署者為 ⓐ 21 列清單之法定效力、ⓑ 正式公告內容與生效時點。
  確認單於 `docs/drafts/HPA-CONFIRMATION-JOB-30.md` 第 4 項（4-A／4-B）。
  **狀態（2026-08-22）：可送出，已交 PI 核閱；v0.10.0 已瘦身**——第 1～3 項改列備查，
  逐項簽核為第 4 項 4-A（21 列法定效力）／4-B（逐列對照）、第 5 項（生效時點），
  以及第 6 項（建請更正原案 30907X 文字，對應 M-12）。
  ⚠️ 送出方式：**由 PI 核閱後改以正式函文格式發出，本團隊不逕行對外發送。**
  ⚠️ **三項實質差異之定案不解除任何擋。** 國健署「嚼檳定案即同意 21 項」仍**僅為口頭／轉述**；
